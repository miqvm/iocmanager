...
// Pintar el llistat de Input Devices
function draw_input_list($input_list)
  {
    echo '
      ...
      // Recorrer cada dispositiu i posar les seves dades 
  ...
  // Pintar el formulari de creacio de Input Devices
  function draw_input_create_form($response=NULL)
  {		
    // Pintar el formulari HTML dels parametres dels diferents metodes
  ...
  // Pintar el formulari d'edicio de Input Devices
  function draw_input_edit($id_input)
  {
    ...
    // Crear el selector de parametres de metodes amb el formulari dinamic
    $methods = $GLOBALS['input']->get_all_input_methods();
    if(count($methods) >= 1)
    {
      $options = '';
