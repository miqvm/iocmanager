...
//TODO hauria d'existir aquesta funcio???
	//--------------------------------------------------------------------------
	// DELETE_IoC
	//--------------------------------------------------------------------------
	//	INPUT:	id_ioc
	...
	//
	//--------------------------------------------------------------------------
	function delete_ioc($id_ioc)
	{
		$id_ioc = preg_replace("[^A-Za-z0-9_]", "", $id_ioc);
		
		// Delete all reasons
		$sql_script = "DELETE FROM reason WHERE ioc_id='$id_ioc'";
		if(!$this->bd->query($sql_script))
		{
			return FALSE;
		}
		$sql_script = "DELETE FROM ioc WHERE id_ioc='$id_ioc'";
		return $this->bd->query($sql_script);
	}
